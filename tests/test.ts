...
describe("GET /", () => {
  it("should return 200 OK", async () => {
    const response = await request(app).get(
      `/weather?lat=${goodCoords[0]}&lon=${goodCoords[1]}`
    );
    expect(response.status).toBe(200);
  });

  it("should return 400 for no coordinates input", async () => {
  ...
  it("should return 422 for out of bounds coordinates", async () => {
    const response = await request(app).get(
      `/weather?lat=${outOfBoundsCoords[0]}&lon=${outOfBoundsCoords[1]}`
    );
  ...
  it("should return 422 for incorrectly formatted coordinates", async () => {
    const response = await request(app).get(
      `/weather?lat=${wrongFormatCoords[0]}&lon=${wrongFormatCoords[1]}`
    );
    expect(response.status).toBe(422);
    expect(response.text).toBe(
      "ERROR: Invalid latitude or longitude format. Please verify latitude and longitude are in decimal degrees notation."
